...
def compute_changed_bibcodes(root_dir='logs/input/'):
    ...
    for x in data_files:
        ...
        #          find changes  | remove comm leading tab, blanks|only bib  | dedup| filter out non-canonical  | current, previous, output file, today's canonical bibcodes
        command = "comm -3 {} {} | sed -e $'s/^[ \t]*//g' -e '/^$/d' | cut -f 1 | uniq | comm -1 -2 - {}  > {}".format(c, p, root_dir + '/current/' + data_files['canonical']['path'], changed_bibs)
